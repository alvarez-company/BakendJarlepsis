...
async function run() {
  ...
  const migrationFile = path.resolve(__dirname, '../src/migrations/2025-12-06_create_asignaciones_tecnicos_table.sql');
  ...
  try {
    ...
    try {
      ...
      if (err.code === 'ER_TABLE_EXISTS_ERROR') {
        console.log(`⚠️  La tabla ya existe: ${path.basename(migrationFile)}`);
      } else {
        throw err;
      }
    }
