...
async function run() {
  ...
  const migrations = [
    path.resolve(__dirname, '../src/migrations/add_movimiento_codigo.sql'),
    path.resolve(__dirname, '../src/migrations/add_traslado_codigo.sql'),
  ];
    ...
    for (const migrationFile of migrations) {
      ...
      try {
        ...
        if (err.code === 'ER_DUP_FIELDNAME') {
          console.log(`⚠️  La migración ya fue aplicada: ${path.basename(migrationFile)}`);
        } else {
          throw err;
        }
      }
